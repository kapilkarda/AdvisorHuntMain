...
class AuthController extends BaseController
{
	...
	public $freeAccessActions = ['userlogin', 'login', 'logout', 'confirm-registration-email'];
	...
	public function actionLogin()
	{
		...
		return $this->renderIsAjax('adminlogin', compact('model'));
	}
	/**
	 * Login form
	 *
	...
	public function actionUserlogin()
	{
		...
		if ( $model->load(Yii::$app->request->post()) AND $model->login() )
		{
			...
			// print_r($user->id);die();
			$moel =User::findOne($user->id);

			$moel->last_login_date =  date("Y-m-d H:i:s");
			$moel->save();
			// return $this->redirect('profile/index');
			if(User::hasRole(['Customer']))
			{
				return $this->redirect(Yii::$app->urlManager->createUrl("profile/index"));
			}else
			if(User::hasRole(['Provider'])){
				return $this->redirect(Yii::$app->urlManager->createUrl("professional/index"));
			}
			else{
				return $this->goBack();
			}			
	...
	public function actionConfirmEmail()
	{
		...
		if ( $model->load(Yii::$app->request->post()) AND $model->validate() )
		{
			if ( $this->triggerModuleEvent(UserAuthEvent::BEFORE_EMAIL_CONFIRMATION_REQUEST, ['model'=>$model]) )
			{
				...
				{
					Yii::$app->session->setFlash('danger', UserManagementModule::t('front', "Unable to send message for email provided"));
				}
			}
		}
