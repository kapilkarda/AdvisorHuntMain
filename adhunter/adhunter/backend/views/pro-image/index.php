...
/* @var $searchModel backend\models\ProimageSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = 'Proimages';
...
<div class="box box-primary">
 ...
 // $data=ArrayHelper::map(City::find()->all(),  'name','id');
  ...
  $gridColumns = [  
  	...
  	[
        'attribute'=>'fk_i_pro_user_id', 
        'vAlign'=>'middle',
        'width'=>'20%',
        'value'=>function ($model, $key, $index, $widget) { 
            // return Company::findOne($model->fk_i_pro_user_id)->name;
           return User::findOne($model->fk_i_pro_user_id)->email; 
        },
        'filterType'=>GridView::FILTER_SELECT2,
       'filter'=>ArrayHelper::map(\Yii::$app->db->createCommand('SELECT u.id, u.email FROM user as u WHERE u.dt_deleted_at IS NULL')->queryAll(), 'id', 'email'),            
        'filterWidgetOptions'=>[
...
?>   
 ...
 <?php echo GridView::widget([
    ...
    'containerOptions'=>['class' => 'hotel-pjax-container', 'style'=>'overflow: auto'], // only set when $responsive = false
    ...
    'panel'=>[
    	...
    	'heading'=>'<h3 class="panel-title"><i class="glyphicon glyphicon-globe"></i> Pro Images</h3>',
    	'footer'=>false
    ],
    'persistResize'=>false,
    'exportConfig'=>true,
...
<div id="hide" class="hide">
    <?= Html::beginForm(['pro-image/activate', 'id'=>''], 'post',array('id'=>'project-image-select-form-activate') )?>
            <?= Html::input('hidden', 'records', '', ['class'=>'selected-records form-control input-md', 'readonly'=>'readonly']) ?>
    <?= Html::endForm() ?>
</div>

<div id="hide" class="hide">
    <?= Html::beginForm(['pro-image/deactivate', 'id'=>''], 'post',array('id'=>'project-image-select-form-deactivate') )?>
            <?= Html::input('hidden', 'records', '', ['class'=>' selected-records form-control input-md', 'readonly'=>'readonly']) ?>
    <?= Html::endForm() ?>
</div>
