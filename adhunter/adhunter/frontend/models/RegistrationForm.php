...
class RegistrationForm extends Model
{
	...
	public function rules()
	{
		...
		if ( Yii::$app->getModule('user-management')->useEmailAsLogin )
		{
			...
			$rules[] = ['username', 'validateEmailConfirmedUnique'];
		}
		else
		{
	...
	public function validateEmailConfirmedUnique()
	{
		if ( $this->username )
		{
			...
			if ( $exists )
			{
				$this->addError('username', 'This E-mail has already been taken.');
			}
		}
	}
	/**
	...
	{
		return [
			'first_name'      => UserManagementModule::t('front', 'First Name'),
			'last_name' 	  => UserManagementModule::t('front', 'Last Name'),
	...
	public function registerUser($performValidation = true)
	{
		...
		$user = new User();
		if(isset($_POST['RegistrationForm']['first_name']))
			$user->first_name = $_POST['RegistrationForm']['first_name'];
		if(isset($_POST['RegistrationForm']['last_name']))
			$user->last_name = $_POST['RegistrationForm']['last_name'];
